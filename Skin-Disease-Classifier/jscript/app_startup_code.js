...
(async function () {
	try {
		// Load local model to avoid network issues
		const MODEL_URL = './final_model_kaggle_version1/model.json';
		...
		$("#prediction-list").append('<li style="list-style-type:none;color:red;">Model failed to load. Check your internet connection or local model files.</li>');
	}
})();
...
$("#predict-button").click(async function () {
	if (!modelReady || !model) {
		$("#prediction-list").empty();
		$("#prediction-list").append('<li style="list-style-type:none;color:#ff3157;">Model is still loading. Please wait a few seconds and try again.</li>');
		return;
	}
